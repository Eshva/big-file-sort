...
internal class PartsBuilder4 : IPartBuilder
{
  public PartsBuilder4(
  ...
  public List<FileInfo> BuildSortedParts(StreamReader inputFileReader)
  {
    ...
    try
    {
      ...
      {
        var readChars = FillBuffer(inputFileReader, buffer);
        var bufferSpan = buffer.Memory.Span.Slice(start: 0, readChars);

        var lastEndOfLineIndex = bufferSpan.LastIndexOfAny(value0: '\r', value1: '\n');
        if (lastEndOfLineIndex == readChars - 1 && bufferSpan[readChars - 2] != '\r' && bufferSpan[readChars - 2] != '\n')
        {
          // Buffer ends with CR or LF. To be sure the next buffer will not start from CR or LF make the current shorter.
          bufferSpan = buffer.Memory.Span.Slice(start: 0, readChars - 1);
          lastEndOfLineIndex = bufferSpan.LastIndexOfAny(value0: '\r', value1: '\n');
        }

        var partFile = new FileInfo(Path.Combine(_workingFolderPath, $"part{_currentPartNumber++}"));
        BuildPart(partFile, bufferSpan.Slice(start: 0, lastEndOfLineIndex + 1));
        var seekBackLength = readChars - lastEndOfLineIndex - 1;
  ...
  private readonly IBufferSorter _sorter = new LinesMinHeapSorter(new BufferLineComparer());
